...
class UnreliableChannel(object):
	...
	def udt_send(self,packt_to_be_sent):
		...
		self.logger.debug("TIME:"+str(self.env.now)+str(self.name)+": udt_send called for"+str(packt))
	...
	def deliver_packet_over_channel(self, delay, packt_to_be_delivered):
		...
		if random.random()<self.Pl:
			self.logger.debug("TIME:"+str(self.env.now)+str(self.name)+":"+str(packt)+"was lost!")
		else:
			# Is this packet corrupted?
			if random.random()<self.Pc:
				packt.corrupt()
				self.logger.debug("TIME:"+str(self.env.now)+str(self.name)+":"+str(packt)+"was corrupted!")
